...
subtitle_label <- get_test_label(res.aov, detailed = TRUE, type = "expression")
...
P1_c <- ggplot(agg2, aes(x = time, y = data_log, col = treat)) +  # Set up the ggplot with data and aesthetics
  ...
  ylab(bquote("log"[10] * "(cells mL" ^ -1 * ")")) +  # Set y-axis label with mathematical expression
  ...
  labs(subtitle = bquote(paste(bold("A) "), .(subtitle_label)))) +  # Add subtitle with expression label
  
  # Add segments for annotations
  geom_segment(  # Add segment for first annotation
    aes(
      x = 14.7,  # Set x-coordinate
      xend = 14.7,  # Set x-coordinate for the end of the segment
      y = 2.3,  # Set y-coordinate
      yend = 2.5  # Set y-coordinate for the end of the segment
    ),
    linewidth = 0.5,  # Set width of the segment
    arrow = arrow(length = unit(0.15, "cm")),  # Add arrow at the end of the segment
    colour = "black",  # Set color of the segment
    show.legend = FALSE  # Hide legend for this layer
  ) +
  geom_segment(  # Add segment for second annotation
    aes(
      x = 16.7,  # Set x-coordinate
      xend = 16.7,  # Set x-coordinate for the end of the segment
      y = 3.2,  # Set y-coordinate
      yend = 3.4  # Set y-coordinate for the end of the segment
    ),
    linewidth = 0.5,  # Set width of the segment
    arrow = arrow(length = unit(0.15, "cm")),  # Add arrow at the end of the segment
    colour = "black",  # Set color of the segment
...
# ANOVA test results as subtitle label
subtitle_label2 <-
  get_test_label(res.aov2, detailed = TRUE, type = "expression")
...
P2_c <- ggplot(agg3, aes(x = time, y = data, col = treat)) +  # Set up the ggplot with data and aesthetics
  ...
  scale_x_continuous(breaks = seq(0, 30, 5)) +  # Set x-axis breaks
  labs(subtitle = bquote(paste(bold("B) "), .(subtitle_label2)))) +  # Add subtitle with expression label
  
  # Add segments for annotations
  geom_segment(  # Add segment for first annotation
    aes(
      x = 14.1,  # Set x-coordinate
      xend = 14.1,  # Set x-coordinate for the end of the segment
      y = 2.18,  # Set y-coordinate
      yend = 2.38  # Set y-coordinate for the end of the segment
    ),
    linewidth = 0.5,  # Set width of the segment
    arrow = arrow(length = unit(0.15, "cm")),  # Add arrow at the end of the segment
    colour = "black",  # Set color of the segment
  ...
  geom_segment(  # Add segment for second annotation
    aes(
      x = 16,  # Set x-coordinate
      xend = 16,  # Set x-coordinate for the end of the segment
      y = 3.85,  # Set y-coordinate
      yend = 3.65  # Set y-coordinate for the end of the segment
    ),
    linewidth = 0.5,  # Set width of the segment
    arrow = arrow(length = unit(0.15, "cm")),  # Add arrow at the end of the segment
    colour = "black",  # Set color of the segment
...
all_plots <-
  P1_c + P2_c + plot_layout(guides = "collect", ncol = 1) &
