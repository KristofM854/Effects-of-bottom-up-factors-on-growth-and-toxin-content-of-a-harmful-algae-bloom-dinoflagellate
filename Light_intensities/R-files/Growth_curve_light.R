...
P1_c <- ggplot(agg, aes(x = time, y = data, col = treat)) +
  ...
  xlab("") +
  ylab(bquote("Cell density (log"[10] * "(cells mL" ^ -1 * "))")) +
  theme_classic() +
  theme(
...
# Format the statistic with two digits after the decimal point
formatted_statistic2 <- sprintf("%.2f", res.aov2$F[3])

# Create subtitle with markdown syntax
subtitle_label2 <-
  glue::glue(
    "<b>b)</b> ANOVA, <i>F</i><sub>{res.aov2$DFn[3]},{res.aov2$DFd[3]}</sub> = {formatted_statistic2}, <i>p</i> < 0.0001"
  )
...
# Format the statistic with two digits after the decimal point
formatted_statistic3 <- sprintf("%.2f", res.aov3$F[3])

# Create subtitle with markdown syntax
subtitle_label3 <-
  glue::glue(
    "<b>c)</b> ANOVA, <i>F</i><sub>{res.aov3$DFn[3]},{res.aov3$DFd[3]}</sub> = {formatted_statistic3}, <i>p</i> < 0.0001"
  )
...
P3_c <- ggplot(agg, aes(x = time, y = data, col = treat)) +
  ...
  coord_cartesian(ylim = c(1.5, 4.0), xlim = c(0, 25)) +
  labs(subtitle = subtitle_label3) +
  guides(col = guide_legend(nrow = 1))

# combine plots of all three strains of A. pseudogonyaulax and export ################
all_plots <-
  P1_c + P2_c + P3_c + plot_layout(guides = "collect", ncol = 1, axis_titles = "collect_y") &
