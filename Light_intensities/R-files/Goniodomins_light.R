...
statistic_label <-
  glue::glue(
    "<b>A) </b>Kruskal-Wallis, <i>H</i>({res.aov1$df}) = {formatted_statistic}, <i>p</i> = {res.aov1$p}, <i>n</i> = {res.aov1$n}"
  )
...
P1 <-  ggplot(Stat, aes(x = treat, y = data1, group = col)) +
  ...
  scale_y_continuous(expression("GDA (pg cell " ^ -1 * ")"), breaks = seq(0, 12.5, 2.5)) +
...
Pex1_2D2 <- ggplot(Stat, aes(y = data1, x =  factor(group))) +
  ...
  scale_y_continuous(expression("GDA (pg cell " ^ -1 * ")")) +
  xlab("") +
  labs(subtitle = "A) per cell:")
...
Pex2_2D2 <- ggplot(Stat, aes(y = GDA_C, x = factor(group))) +
  ...
  scale_x_discrete(labels = c(20, 100, 100, 200, 200)) +
  scale_y_continuous(expression("GDA mol:mol (GDA C" ^ -1 * ")"),
                     label = scientific_format(digits = 2)) +
  xlab("") +
  labs(subtitle = "A) per carbon:")
...
Pex3_2D2 <- ggplot(Stat, aes(y = tox_vol, x = factor(group))) +
  ...
  scale_x_discrete(labels = c(20, 100, 100, 200, 200)) +
  scale_y_continuous(expression(paste("GDA (pg ", mu, "m" ^ -3 *
                                        ")")), label = scientific_format(digits = 2)) +
  xlab("") +
  labs(subtitle = "A) per volume:")
...
# Manually create the subtitle with markdown syntax using glue
statistic_label2 <- glue::glue("<b>B) </b>Kruskal-Wallis, <i>H</i>({res.aov2$df}) = {formatted_statistic}, <i>p</i> = {res.aov2$p}, <i>n</i> = {res.aov2$n}")
...
statistic_label3 <-
  glue::glue(
    "<b>C) </b>Kruskal-Wallis, <i>H</i>({res.aov3$df}) = {formatted_statistic}, <i>p</i> = {res.aov3$p}, <i>n</i> = {res.aov3$n}"
  )
...
pval_P3 <- pval_P3 %>% mutate(group1 = c("100", "200"), group2 = c("100", "200"))
  ...
  xlab(expression(paste(
    "Photon flux densities (", mu, "mol photons m" ^ -2 * "s" ^ -1 * ")"
  ))) +
  scale_y_continuous(expression("GDA (pg cell " ^ -1 * ")"), breaks = seq(0, 12.5, 2.5)) +
  scale_x_discrete(labels = c(20,  100, 200)) +
  coord_cartesian(ylim = c(0, 14)) +
...
all_plots2 <-
  Pex1_2D2 + Pex3_2D2  + Pex1_4B1 + Pex3_4B1  + Pex1_4B9 + Pex3_4B9  +
...
# manually change scaling
ragg::agg_png("C:/Users/krist/OneDrive/Dokumente/AWI/Promotion/AP1/Light-AP1b/figures/Toxins_comparison.png", width = 20, height = 20, units = "in", res = 300, scaling = 2
)
