##########################################
## AP1 N-growth Experiment; GDA plots of A pseudogonyaulax strains L2-D2 and L4-B1
...
statistic_label <-
  glue::glue(
    "<b>B) </b>Kruskal-Wallis, <i>H</i>({res.aov3$df}) = {formatted_statistic}, <i>p</i> = {res.aov3$p}, <i>n</i> = {res.aov3$n}"
  )
...
all_plots <-
  P1b + P2a + plot_layout(guides = "collect", ncol = 1) &
...
all_plots_comparison <-
  Pex1 + Pex3 + Pex2 +  Pex1_4B1 + Pex3_4B1 + Pex2_4B1 + plot_layout(guides = "collect", ncol = 3) &
  theme(legend.position = "bottom",
        legend.box.margin = margin(-20, -20, -10, -20))
